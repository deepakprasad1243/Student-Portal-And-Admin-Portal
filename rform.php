...
$mamount=mysqli_real_escape_string($con,$_POST['mamount']);
$mtransaction=mysqli_real_escape_string($con,$_POST['mtransaction']);
$mdate=mysqli_real_escape_string($con,$_POST['mdate']);
$back=mysqli_real_escape_string($con,$_POST['backlogs']);
$subject=$_POST['subject'];
$sub=implode(",",$subject);
$photo = $_FILES['photo']['name'];
$imageInstitute = $_FILES['imageInstitute']['name'];
$imageMess = $_FILES['imageMess']['name'];



if(isset($_POST['update']))
{
	$sql="SELECT `Photo`,`Institute fee Reciept`,`Mess fee reciept` FROM  `registration` WHERE `Roll`= '$roll'";
	        
    $result=mysqli_query($con,$sql) or die("error ->  ".mysqli_error($con));
	$row=mysqli_fetch_array($result);
	        $photoPre = $row['Photo'];
			$imageInstitutePre = $row['Institute fee Reciept']; 
			$imageMessPre = $row['Mess fee reciept'];
	if($photo=="")
	{
		$photo = $photoPre;
	}
	if($imageInstitute=="")
	{
		$imageInstitute = $imageInstitutePre;
	}
	if($imageMess=="")
	{
		$imageMess = $imageMessPre;
	}
}
...
if(isset($_FILES['imageInstitute']['name']))
	{
		$target1 = 'images/'.basename($_FILES['imageInstitute']['name']);
        //$imageInstitute = $_FILES['imageInstitute']['name'];
        move_uploaded_file($_FILES['imageInstitute']['tmp_name'],$target1);
	}
if(isset($_FILES['imageMess']['name']))
	{
		$target2 = 'images/'.basename($_FILES['imageMess']['name']);
        //$imageMess = $_FILES['imageMess']['name'];
        move_uploaded_file($_FILES['imageMess']['tmp_name'],$target2);
	}
...
if(isset($_POST['update']))
{
	$add2=mysqli_real_escape_string($con,$_POST['address']);
    
	
	$sql="UPDATE `registration` SET `Category`='$category',`Course`='$course',`Photo`='$photo',`Semister`='$semister',`Year`='$year',`Branch`='$branch',`Mobile No.`='$number',`Father's Name`='$fname',`Father's mobile No.`='$fnumber',`SGPA 1`='$sgpa1',`Amount`='$amount',`Transaction No.`='$transaction',`transaction Date`='$date',`messAmount`='$mamount',`messTransaction No.`='$mtransaction',`messTransaction Date`='$mdate',`Backlogs`='$back',`Subject(s) opted {semister}`='$sub' ,
	`Institute fee Reciept`='$imageInstitute',`Mess fee Reciept`='$imageMess'   WHERE  `Roll`= '$roll'";
	mysqli_query($con,$sql) or die("error in updation".mysqli_error($con));
	
	
	$sql="UPDATE `new registration` SET `Semister`='$semister',`Year`='$year',`Mobile No.`='$number',`Father's mobile No.`='$fnumber',`Corresponding Address`='$add2' WHERE  `Roll`= '$roll'";
	mysqli_query($con,$sql) or die("error in updation".mysqli_error($con));
...
{
$sql="INSERT INTO `registration`(`Name`, `Roll`,`Photo`, `Email id`, `Category`, `Course`, `Semister`, `Year`, `Branch`, `Mobile No.`, `Father's Name`, `Father's mobile No.`, `SGPA 1`,  `Amount`, `Transaction No.`, `transaction Date`, `messAmount`, `messTransaction No.`, `messTransaction Date`,`Backlogs`,`Subject(s) opted {semister}`,`Institute fee Reciept`,`Mess fee reciept`)
 VALUES ('$name','$roll','$photo','$email','$category','$course','$semister','$year','$branch','$number','$fname','$fnumber','$sgpa1','$amount','$transaction','$date','$mamount','$mtransaction','$mdate','$back','$sub','$imageInstitute','$imageMess')";
